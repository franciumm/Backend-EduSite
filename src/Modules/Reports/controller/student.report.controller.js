...
function streamPdf(res, { summary, assignments, exams }) {
  ...
  const H = (t, s = 18) => { doc.fontSize(s).text(t); };
  const L = (left, right) => {
    doc.fontSize(11).text(`${left}`, { continued: true }).text(`  ${right}`);
  };
  const line = () => doc.moveDown(0.5).strokeColor("#aaa").moveTo(40, doc.y).lineTo(555, doc.y).stroke().moveDown(0.5);
  ...
  H("Summary", 16);
  doc.moveDown(0.3);
  L("Assignments:", `total=${summary.assignments.total}, marked=${summary.assignments.marked}, late=${summary.assignments.late}, missing=${summary.assignments.missing}, avgScore=${summary.assignments.avgScore ?? "-"}`);
  ...
  writeTable(
    "Exams",
    ["name", "version", "submittedAt", "score", "notes"],
    exams.map(e => ({
      name: e.name,
      version: e.version,
...
export const generateStudentReport = asyncHandler(async (req, res, next) => {
  ...
  } = req.body; // JoiValidation merges params/query/body
