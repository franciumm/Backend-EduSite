...
// --- CORRECTED `removeStudent` FUNCTION ---
export const removeStudent = asyncHandler(async (req, res, next) => {
    const { groupid, studentid } = req.body;

    // 1. Pre-validation Step (Outside the transaction)
    const [groupExists, studentExists] = await Promise.all([
        groupModel.findById(groupid).lean(),
        studentModel.findById(studentid).lean()
    ]);

    if (!groupExists) return next(new Error('Invalid Group ID', { cause: 400 }));
    if (!studentExists) return next(new Error('Student not found', { cause: 404 }));
...
export const addStudentsToGroup = asyncHandler(async (req, res, next) => {
    ...
    const [groupExists, students] = await Promise.all([
        groupModel.findById(groupid).lean(),
        studentModel.find({ _id: { $in: uniqueStudentIds } })
    ]);
    ...
    if (students.length !== uniqueStudentIds.length) {
        return next(new Error("One or more students not found", { cause: 404 }));
    }
...
export const groupDelete = asyncHandler(async (req, res, next) => {
    ...
    // 1. Pre-validation Step
    const groupExists = await groupModel.findById(groupid).lean();
    if (!groupExists) return next(new Error('Group not found', { cause: 404 }));
    ...
    try {
        ...
        // 3. Perform actions inside the transaction
...
// Other functions remain the same as they were correct...
